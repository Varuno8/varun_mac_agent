...
enum BuddyPushToTalkShortcut {
    ...
    enum ShortcutTransition {
        case none
        /// `isNewTask=true` when the user pressed the "new task" variant
        /// (ctrl + option) — CompanionManager clears conversationHistory
        /// before starting the recording. `isNewTask=false` when the user
        /// pressed the "continue task" variant (cmd + option) — history is
        /// preserved so multi-turn tasks (e.g. corrections after a mistake)
        /// share full context.
        case pressed(isNewTask: Bool)
        case released
    }
    ...
    /// Modifier-only flags that start a NEW task (history cleared before recording).
    static let newTaskModifierFlags: NSEvent.ModifierFlags = [.control, .option]
    /// Modifier-only flags that CONTINUE the current task (history preserved).
    static let continueTaskModifierFlags: NSEvent.ModifierFlags = [.command, .option]

    static let pushToTalkKeyCode: UInt16 = 49 // Space
    static let pushToTalkDisplayText = "ctrl+opt (new task) · cmd+opt (continue)"
    static let pushToTalkTooltipText = "push to talk — ctrl+opt for a new task, cmd+opt to continue the current one"
    ...
    private static func shortcutTransition(
        ...
        // Both push-to-talk shortcuts are modifier-only (no space key), so we
        // only care about flagsChanged events. The two variants are:
        //   • newTaskModifierFlags     ([.control, .option]) — clears history
        //   • continueTaskModifierFlags ([.command, .option]) — preserves history
        // Whichever flag set is active when the press transition fires
        // determines `isNewTask`. If the user holds *both* sets simultaneously
        // (which would require a four-modifier press, ctrl+option+cmd), we
        // bias toward "new task" — that's the safer default since clearing
        // history is recoverable while clobbering the wrong recipient isn't.
        guard shortcutEventType == .flagsChanged else { return .none }

        let isNewTaskPressed = modifierFlags.contains(newTaskModifierFlags)
        let isContinueTaskPressed = modifierFlags.contains(continueTaskModifierFlags)
        let isAnyShortcutPressed = isNewTaskPressed || isContinueTaskPressed

        if isAnyShortcutPressed && !wasShortcutPreviouslyPressed {
            return .pressed(isNewTask: isNewTaskPressed)
        }
